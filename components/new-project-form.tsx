...
export function NewProjectForm() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      router.push(`/projects/${created.project.id}?analyze=1`);
    } catch (caught) {
  ...
  return (
    <main className="new-project-page">
      ...
      <div className="new-project-heading"><span className="kicker">New renovation</span><h1>Start with the space<br />as it is today.</h1><p>A clear, well-lit photograph gives the design brain the strongest architectural context.</p></div>
        ...
        <section className="upload-stage-card">
          {preview ? (
            ...
            <div className={`drop-zone large ${dragging ? "dragging" : ""}`} onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); choose(event.dataTransfer.files[0]); }} onClick={() => inputRef.current?.click()}>
              <span className="upload-icon"><Upload size={25} /></span><h2>Drop your space here</h2><p>or choose a photo from your device</p><button type="button" className="button button-secondary"><ImagePlus size={17} /> Choose photo</button><small>JPEG, PNG, WebP, AVIF or GIF · max 10 MB</small><span className="camera-hint"><Camera size={15} /> Camera capture supported on mobile</span>
            </div>
          )}
          <input ref={inputRef} className="sr-only" type="file" accept="image/jpeg,image/png,image/webp,image/avif,image/gif" capture="environment" onChange={(event) => choose(event.target.files?.[0])} />
        </section>
